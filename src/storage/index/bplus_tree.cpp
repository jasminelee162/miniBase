...
page_id_t BPlusTree::CreateNew() {
    ...
    root_page_id_ = pid;
    // 持久化根
    if (engine_) engine_->SetIndexRoot(root_page_id_);
    return pid;
}
...
void BPlusTree::LoadRootFromStorage() {
    ...
    if (saved != INVALID_PAGE_ID) root_page_id_ = saved;
}
...
int BPlusTree::FindChildIndex(Page* parent, page_id_t left_child_id) {
    ...
    for (uint16_t i = 0; i < nh->key_count + 1; ++i) {
        if (ia.children[i] == left_child_id) return static_cast<int>(i);
    }
    return -1;
}
...
bool BPlusTree::InsertIntoInternal(Page* parent, int insert_pos, int32_t key, page_id_t right_child) {
    ...
    uint16_t n = nh->key_count;
    if (n >= GetInternalMaxKeys()) return false; // 需要分裂
...
void BPlusTree::SplitInternalAndPropagate(Page* parent, int32_t key, page_id_t right_child) {
    ...
    int pos = 0;
    while (pos < nhc->key_count && key > iac.keys[pos]) ++pos;
    if (InsertIntoInternal(parent, pos, key, right_child)) return;
    ...
    for (uint16_t i = 0; i < n; ++i) keys[i] = ia.keys[i];
    for (uint16_t i = 0; i < n + 1; ++i) children[i] = ia.children[i];
    int insert_pos = 0;
    while (insert_pos < n && key > keys[insert_pos]) ++insert_pos;
    for (int i = static_cast<int>(n); i > insert_pos; --i) keys[i] = keys[i - 1];
    for (int i = static_cast<int>(n + 1); i > insert_pos + 1; --i) children[i] = children[i - 1];
    keys[insert_pos] = key;
    ...
    for (uint16_t i = 0; i < mid; ++i) ia.keys[i] = keys[i];
    for (uint16_t i = 0; i < mid + 1; ++i) ia.children[i] = children[i];
    nh->key_count = mid;

    // 创建右内节点
    page_id_t right_id = INVALID_PAGE_ID;
    Page* right_node = engine_->CreatePage(&right_id);
    if (!right_node) { engine_->PutPage(parent->GetPageId(), true); return; }
    ...
    for (uint16_t i = 0; i < right_keys; ++i) ria.keys[i] = keys[mid + 1 + i];
    for (uint16_t i = 0; i < right_keys + 1; ++i) ria.children[i] = children[mid + 1 + i];
...
void BPlusTree::InsertIntoParent(page_id_t left_child, page_id_t right_child, int32_t separator_key) {
    ...
    engine_->PutPage(left_child, false);
    if (parent_id == INVALID_PAGE_ID) return;
    Page* parent = engine_->GetPage(parent_id);
    if (!parent) return;
    int idx = FindChildIndex(parent, left_child);
    if (idx < 0) { engine_->PutPage(parent_id, false); return; }
...
int32_t BPlusTree::GetLeafFirstKey(const Page* leaf) const {
    const NodeHeader* nh = GetNodeHeaderConst(leaf);
    if (nh->key_count == 0) return INT32_MIN;
    const LeafEntry* arr = GetLeafEntriesConst(leaf);
    return arr[0].key;
}
...
void BPlusTree::RemoveParentKey(page_id_t parent_id, int key_index) {
    ...
    uint16_t n = nh->key_count;
    for (int i = key_index; i < static_cast<int>(n) - 1; ++i) ia.keys[i] = ia.keys[i + 1];
    for (int i = key_index + 1; i < static_cast<int>(n); ++i) ia.children[i] = ia.children[i + 1];
    nh->key_count = static_cast<uint16_t>(n - 1);
    engine_->PutPage(parent_id, true);
}
...
void BPlusTree::RebalanceLeaf(Page* leaf) {
    ...
    if (nh->key_count >= min_entries || leaf->GetPageId() == root_page_id_) return;
    ...
    if (left_sib_id != INVALID_PAGE_ID) {
        ...
        if (lnh->key_count > min_entries) {
            ...
            LeafEntry* arr = GetLeafEntries(leaf);
            // 将 leaf 中元素整体右移一位
            for (int i = nh->key_count; i > 0; --i) arr[i] = arr[i - 1];
    ...
    if (right_sib_id != INVALID_PAGE_ID) {
        ...
        if (rnh->key_count > min_entries) {
            ...
            for (uint16_t i = nh->key_count; i > 0; --i) { /* keep order */ }
            arr[nh->key_count] = rarr[0];
            nh->key_count++;
            // 右 sibling 左移
            for (uint16_t i = 0; i < rnh->key_count - 1; ++i) rarr[i] = rarr[i + 1];
            rnh->key_count--;
            // 父分隔键替换为 right 新首键
    ...
    if (left_sib_id != INVALID_PAGE_ID) {
        ...
        NodeHeader* lnh = GetNodeHeader(left);
        // 将 leaf 全部搬到 left 尾部
        for (uint16_t i = 0; i < nh->key_count; ++i) larr[lnh->key_count + i] = arr[i];
        lnh->key_count = static_cast<uint16_t>(lnh->key_count + nh->key_count);
        // 调整链表
        lnh->next = nh->next;
        if (nh->next != INVALID_PAGE_ID) {
            Page* nxt = engine_->GetPage(nh->next);
            if (nxt) { GetNodeHeader(nxt)->prev = left_sib_id; engine_->PutPage(nxt->GetPageId(), true); }
        }
        engine_->PutPage(left_sib_id, true);
    ...
    if (right_sib_id != INVALID_PAGE_ID) {
        ...
        for (uint16_t i = 0; i < rnh->key_count; ++i) arr[nh->key_count + i] = rarr[i];
        nh->key_count = static_cast<uint16_t>(nh->key_count + rnh->key_count);
        // 调整链表
        nh->next = rnh->next;
        if (rnh->next != INVALID_PAGE_ID) {
            Page* nxt = engine_->GetPage(rnh->next);
            if (nxt) { GetNodeHeader(nxt)->prev = leaf->GetPageId(); engine_->PutPage(nxt->GetPageId(), true); }
        }
        engine_->PutPage(leaf->GetPageId(), true);
...
void BPlusTree::RebalanceInternal(page_id_t node_id) {
    ...
    if (node_id == root_page_id_) {
        ...
        if (nh->key_count == 0) {
            ...
            if (child) { GetNodeHeader(child)->parent = INVALID_PAGE_ID; engine_->PutPage(root_page_id_, true); }
        }
        engine_->PutPage(node_id, true);
        return;
    }
    uint16_t min_keys = GetInternalMinKeys();
    if (nh->key_count >= min_keys) { engine_->PutPage(node_id, false); return; }
    page_id_t parent_id = nh->parent;
    Page* parent = engine_->GetPage(parent_id);
    if (!parent) { engine_->PutPage(node_id, false); return; }
    int idx = FindChildIndex(parent, node_id);
    if (idx < 0) { engine_->PutPage(parent_id, false); engine_->PutPage(node_id, false); return; }
    ...
    if (left_id != INVALID_PAGE_ID) {
        ...
        if (lnh->key_count > min_keys) {
            ...
            for (int i = nh->key_count; i > 0; --i) ia.keys[i] = ia.keys[i - 1];
            for (int i = nh->key_count + 1; i > 0; --i) ia.children[i] = ia.children[i - 1];
    ...
    if (right_id != INVALID_PAGE_ID) {
        ...
        if (rnh->key_count > min_keys) {
            ...
            for (uint16_t i = 0; i < rnh->key_count - 1; ++i) ria.keys[i] = ria.keys[i + 1];
            for (uint16_t i = 0; i < rnh->key_count; ++i) ria.children[i] = ria.children[i + 1];
            rnh->key_count--;
            engine_->PutPage(right_id, true);
            engine_->PutPage(node_id, true);
            engine_->PutPage(parent_id, true);
    ...
    if (left_id != INVALID_PAGE_ID) {
        ...
        for (uint16_t i = 0; i < nh->key_count; ++i) lia.keys[ln + 1 + i] = ia.keys[i];
        for (uint16_t i = 0; i < nh->key_count + 1; ++i) lia.children[ln + 1 + i] = ia.children[i];
        GetNodeHeader(left)->key_count = static_cast<uint16_t>(ln + 1 + nh->key_count);
        // 更新合并孩子的父指针
        for (uint16_t i = 0; i < nh->key_count + 1; ++i) {
            Page* ch = engine_->GetPage(ia.children[i]);
            if (ch) { GetNodeHeader(ch)->parent = left_id; engine_->PutPage(ia.children[i], true); }
        }
        RemoveParentKey(parent_id, idx - 1);
        engine_->PutPage(left_id, true);
    ...
    if (right_id != INVALID_PAGE_ID) {
        ...
        ia.keys[nh->key_count] = GetInternalArraysConst(parent).keys[idx];
        for (uint16_t i = 0; i < rn; ++i) ia.keys[nh->key_count + 1 + i] = ria.keys[i];
        for (uint16_t i = 0; i < rn + 1; ++i) ia.children[nh->key_count + 1 + i] = ria.children[i];
        nh->key_count = static_cast<uint16_t>(nh->key_count + 1 + rn);
        for (uint16_t i = 0; i < rn + 1; ++i) {
            Page* ch = engine_->GetPage(ria.children[i]);
            if (ch) { GetNodeHeader(ch)->parent = node_id; engine_->PutPage(ria.children[i], true); }
        }
        RemoveParentKey(parent_id, idx);
        engine_->PutPage(node_id, true);
        engine_->PutPage(right_id, false);
...
bool BPlusTree::DeleteAndRebalance(int32_t key) {
    ...
    Page* leaf = DescendToLeaf(key);
    if (!leaf) return false;
    if (!DeleteFromLeaf(leaf, key)) return false;
    // 删除后重平衡叶子
    RebalanceLeaf(leaf);
    engine_->PutPage(leaf->GetPageId(), false);
    return true;
